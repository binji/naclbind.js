...
var defaultArches = ['x86_32', 'x86_64', 'arm'];
...
function clone(obj) {
  var result;
  var i;
  var clazz = Object.prototype.toString.call(obj);
...
function splitext(p) {
  ...
  var i = p.lastIndexOf(ext);
  return [p.slice(0, i), ext];
}

function getToolPath(tool, opts, callback) {
  var args;
  var key = [tool, opts.toolchain, opts.arch].join('-');
...
function compile(infiles, outfile, opts, callback) {
  ...
  var getDefaultLibDirs = function() {
        if (opts.toolchain === 'pnacl') {
...
function build(infiles, outname, opts, callback) {
  ...
  var doFinalize = function(infile) {
  ...
  var doTranslate = function(infile) {
        var outfiles = [];
        var pendingArches = arches.length;
  ...
  var doCompileNacl = function() {
        ...
        arches.forEach(function(arch) {
          ...
          compile(infiles, outfileNexe(arch), copts, function(error, outfile) {
            var sopts;
            var nexeReady = function(outfile) {
                  outfiles.push(outfile);
                  if (--pendingArches === 0) {
                    if (opts.createNmf) {
                      doNmf(outfiles);
