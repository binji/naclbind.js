...
function preprocessor(logger, fileList) {
  ...
  return function(content, file, done) {
    ...
    doGenC = function() {
      var genOpts = {template: 'glue.c'};

      log.debug('Generating %s from %s. opts: %s',
                glueC, header, JSON.stringify(genOpts));
    ...
    doGenJS = function() {
      var genOpts = {template: 'glue.js'};

      log.debug('Generating %s from %s. opts: %s',
                glueJS, header, JSON.stringify(genOpts));
      gen.file(header, glueJS, genOpts, function(error, outfile) {
        if (error) {
          log.error('Failed to build: %s', error.toString());
          return done(error);
        }
