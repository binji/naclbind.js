...
function preprocessor(logger, fileList) {
  ...
  return function(content, file, done) {
    ...
    var basename = /.*test_(.*?).js$/.exec(file.path)[1];
    var header = path.join(dirname, basename + '.h');
    var source = path.join(dirname, basename + '.c');
    var outdir = path.join(naclbindDir, 'out/test/integration', basename);
    var glueC = path.join(outdir, 'glue.c');
    var glueJS = path.join(outdir, 'glue.js');
    var outTestJs = path.join(outdir, 'test_' + basename + '.js');
